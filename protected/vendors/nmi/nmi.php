...
// Initial Setting Functions
    ...
    function doAuth1($amount, $ccnumber, $ccexp, $cvv="",$recurring) {
        ...
        //$query .= "plan_id=garciniaautoship1  &";
        $query .= "month_frequency=".intval(1)."&";
        $query .= "plan_payments=0&";

        $query .= "plan_amount=77.90&";
        //$query .= "plan_name= Garcinia".time()."  &";

        //if(date('d')==31)
        //$query .= "day_of_month=30&";
        //else
            $date=strtotime(date('Ymd'));
            $newDate = date('d',strtotime('+15 days',$date));

        $query .= "day_of_month=" .$newDate. " &";
        //$query .= "start_date= 20151111 &";
        ...
        $query .= "shipping_email=" . urlencode($this->shipping['email']) . "&";
        $date=strtotime(date('Ymd'));
        $newDate = date('Ymd',strtotime('+15 days',$date));
        //var_dump($newDate);
        //exit;
        if($recurring==1)
        $query .= "type=auth&start_date=".$newDate;
        else
        $query .= "type=auth";
        //var_dump($query);
        //exit;
